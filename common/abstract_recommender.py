...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            # dataset_path = os.path.abspath(config['data_path'] + config['dataset']) # đưa ra ngoai if
            # if file exist?
        ...
        # thêm user-graph ở đây -------------------------------------------------------------------------------------
        user_graph_file_path = os.path.join(dataset_path, config['user_graph_dict_file'])
        if os.path.isfile(user_graph_file_path):
            # user_graph_dict_old = np.load(user_graph_file_path, allow_pickle=True).item()
            # # tạo dictionary mới có dạng {(i, j): weight}
            # self.user_graph_dict = {}

            # for user_i, (neighbors, weights) in user_graph_dict_old.items():
            #     for user_j, w in zip(neighbors, weights):
            #         self.user_graph_dict[(user_i, user_j)] = float(w)
            self.user_graph_dict = np.load(user_graph_file_path, allow_pickle=True).item()
        else:
