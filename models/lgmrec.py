...
class LGMRec(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if self.user_graph_dict is not None:
            _, self.norm_user_graph = self.get_norm_user_graph()
            # user_graph_matrix = sp.dok_matrix((self.n_users, self.n_users), dtype=np.float32)
            # data_dict = dict(zip(zip(*self.user_graph_dict.keys()), self.user_graph_dict.values()))
            # user_graph_matrix._update(data_dict)
            # self.user_graph = self.scipy_matrix_to_sparse_tenser(user_graph_matrix, torch.Size((self.n_users, self.n_users)))
        # ---------------------------------------------------------------------------------------------------------------------
    ...
    # thêm hàm chuẩn hóa cho user_graph ---------------------------------------------------------------------------------
    def get_norm_user_graph(self):
        # Khởi tạo ma trận kề rỗng (dok_matrix để update nhanh)
        A = sp.dok_matrix((self.n_users, self.n_users), dtype=np.float32)

        # Chuyển user_graph_dict → data_dict {(u,v): weight}
        data_dict = {}
        for u, (nbr_ids, nbr_vals) in self.user_graph_dict.items():
            for v, w in zip(nbr_ids, nbr_vals):
                data_dict[(u, v)] = w
                data_dict[(v, u)] = w   # Đảm bảo tính đối xứng

        # Cập nhật ma trận kề
        A._update(data_dict)

        # Tính bậc của mỗi node (degree)
        sumArr = (A > 0).sum(axis=1)
        diag = np.array(sumArr.flatten())[0] + 1e-7  # tránh chia 0

        # Chuẩn hóa đối xứng: D^{-1/2} * A * D^{-1/2}
        diag = np.power(diag, -0.5)
        D = sp.diags(diag)
        L = D @ A @ D  # L = D^{-1/2} A D^{-1/2}

        # Chuyển sang sparse tensor PyTorch
        L = sp.coo_matrix(L)
        norm_user_graph = self.scipy_matrix_to_sparse_tenser(
            L, torch.Size((self.n_users, self.n_users))
        )

        return sumArr, norm_user_graph
    # -------------------------------------------------------------------------------------------------------------------
    ...
    def cge(self):
        ...
        # Đoạn mới thêm =====
            ...
            raise ValueError(f"Unsupported cf_model: {self.cf_model}")
        # ==================
        return cge_embs
    ...
    def forward(self):
        ...
        if self.v_feat is not None and self.t_feat is not None:
            ...
            # thêm user_graph ---------------------------------------------------------------------------------------------------
            if hasattr(self, 'norm_user_graph'):
                user_embs = cge_embs[:self.n_users]
                # Lan truyền qua user-user graph (LightGCN style)
                # uge_embs = user_embs
                # for _ in range(self.n_ui_layers):  
                #     uge_embs = torch.sparse.mm(self.norm_user_graph, uge_embs)
                # thêm bước mean pooling để tận dụng thông tin từ các tầng lan truyền
                uge_embs = user_embs 
                uge_stack = [user_embs]
                for _ in range(self.n_ui_layers):
                    uge_embs = torch.sparse.mm(self.norm_user_graph, uge_embs)
                    uge_stack.append(uge_embs)
                uge_embs = torch.stack(uge_stack, dim=1).mean(dim=1)
                # Chuẩn hóa kết quả
                uge_embs = F.normalize(uge_embs)
                # Nối lại user-item embedding thành 1 tensor
                uge_all_embs = torch.cat([uge_embs, cge_embs[self.n_users:]], dim=0)
                # Tổng hợp local graph embedding từ 3 nguồn
                lge_embs = cge_embs + mge_embs + self.alpha * uge_all_embs
            else:
